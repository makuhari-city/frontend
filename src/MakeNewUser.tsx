  ...
  return (
    <>
      ...
      <div className="container mx-auto px-6 py-16 max-w-screen-lg bg-nord-0 bg-opacity-90 rounded">
        ...
        <div className="font-thin text-xs mb-8">
          幕張市プロジェクト参加には住民登録が必要です。
        </div>
        ...
        {isUserSaved && (
          <div className="w-3/5 grid grid-justify-items-end">
            <div className="text-nord-9 my-4">
              '{newUser!.name}' created with id{" "}
              <span style={{ color: `${{ color }}` }}>
                {newUser!.uid.substring(0, 6)}
              </span>
            </div>
            <Link
              to="/app/list"
              className="ease-in duration-150 bg-nord-1 hover:bg-nord-2 text-nord-4 font-bold py-2 px-6 rounded text-center"
            >
              go to list
            </Link>
          </div>
